...
public final class LyricsStudio {
    ...
    public static func fetchAllLyrics(for id: String) async throws -> [StudioLyricResponse] {
        guard let url = URL(string: "https://taproom.cider.sh/api/v1/lyrics/track/\(id)/all") else { throw LyricStudioError.badURL(nil) }
        ...
        if let http = response.1 as? HTTPURLResponse, http.statusCode > 299 {
            throw LyricStudioError.responseError(String(data: response.0, encoding: .utf8))
        }
